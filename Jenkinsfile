    ...
    stages {       
        stage('Building project, running tests and publishing test results') {
            ...
            post {
                ...
                success {
                    archive "target/fortuneteller-jsf-1.0.war"
                }
            } 
        }         
        ...
        stage('Deploying artifacts to selected target'){
            ...
            steps{
                ...
                sh 'asadmin --port 4848 deploy --force --name fortuneteller-${DEPLOY_ENV} --contextroot fortuneteller-${DEPLOY_ENV} target/fortuneteller-jsf-1.0.war'
            }
        } 
    }
}
